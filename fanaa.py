...
class Lexer:
    ...
    def make_string(self):
        ...
        while self.current_char != None and (self.current_char != '"' or escape_character):
            if escape_character:
                string += escpae_characters.get(self.current_char, self.current_char)
            else:
                if self.current_char == '\\':
...
class IfNode:
    def __init__(self,cases,else_case):
        self.cases = cases #cases is a list with tuple of (condition, expr)
        self.else_case = else_case #will be expression node
        self.pos_start = self.cases[0][0].pos_start
        self.pos_end= self.else_case or self.cases[len(self.cases)-1][0]

class FromNode:
    def __init__(self,var_name_tok,start_value_node,end_value_node,skip_value_node,body_node,should_return_null):
        self.var_name_tok = var_name_tok
        self.start_value_node = start_value_node
        self.end_value_node = end_value_node
        self.skip_value_node = skip_value_node
        self.body_node = body_node
...
class Parser:
    ...
    def parse(self):
        res = self.statements()
        if not res.error and self.current_tok.type != TT_EOF: return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end,"Expected '+', '-', '*', '/', '^', '==', '!=', '<', '>', <=', '>=', 'AND' or 'OR'"))
        return res
    def statements(self):
        res = ParseResult()
        statements = []
    ...
    def from_expr(self):
        ...
        if self.current_tok.matches(TT_KEYWORD,"SKIP"):
            res.register_advancement()
            self.advance()
            skip_value = res.register(self.expr())
            if res.error: return res
        else:
            skip_value = None
        if not self.current_tok.matches(TT_KEYWORD,"DO"):
        ...
        if self.current_tok.type == TT_NEWLINE:
            ...
            res.register_advancement()
            self.advance()
            return res.success(FromNode(var_name, start_value, end_value, skip_value, body,True))                 

        body = res.register(self.statement())
        if res.error: return res
        return res.success(FromNode(var_name, start_value, end_value, skip_value, body,False))                 
    def while_expr(self):
        res = ParseResult()
        ...
        condition = res.register(self.expr())
        if res.error: return res
        if not self.current_tok.matches(TT_KEYWORD,"DO"): return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end,"expected DO"))
        res.register_advancement()
        self.advance()
        if self.current_tok.type == TT_NEWLINE:
            res.register_advancement()
    ...
    def list_expr(self):
        ...
        if self.current_tok.type == TT_RSQUARE:
            ...
            while self.current_tok.type == TT_COMMA:
                ...
                if res.error: return res
            if self.current_tok.type != TT_RSQUARE:
                return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end,"',' or ']'"))
            res.register_advancement()
            self.advance()
    ...
    def func_def(self):
        ...
        if self.current_tok.type == TT_IDENTIFIER:
            ...
            if self.current_tok.type != TT_LPAREN:
                return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end,"Expected '('"))
        else: #function must have name 
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end,"Expected identifier"))
        res.register_advancement()
        self.advance()
        arg_name_toks = []
        if self.current_tok.type == TT_IDENTIFIER: #begin reading parameters
            ...
            if self.current_tok.type != TT_RPAREN:
                return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end,"Expected Parameter Identifiers"))
        res.register_advancement()
        self.advance()
...
class List(Value):
    ...
    def added_to(self,others):
        new_list = self.copy()
        new_list.elements.append(others)
    ...
    def subbed_by(self,other):
        if isinstance(other,Number):
            ...
            try:
                ...
                return None, RTError(other.pos_start,other.pos_end, "Index is out of bounds",self.context)
        else:
...
class BaseFunction(Value):
    ...
    def check_args(self,arg_names,args):
        ...
        if len(args) != len(arg_names):
            return res.failure(RTError(self.pos_start,self.pos_end,f"Expected {len(arg_names)} arguments but got {len(args)}",self.context))
...
class BuiltInFunction(BaseFunction):
    ...
    def no_visit_method(self,node,context):
        raise Exception(f'No execute_{self_name} method defined')
    def copy(self):
    ...
    def execute_pop(self,exec_ctx):
        ...
        if not isinstance(list_,List):
            return RTResult().failure(RTError(self.pos_start,self.pos_end,"Tried to use pop on something that was not a list",exec_ctx))
        if not isinstance(index,Number):
            return RTResult().failure(RTError(self.pos_start,self.pos_end,"Second argument must be a number ",exec_ctx))
        try:
    ...
    def execute_run(self,exec_ctx):
        ...
        try:
            ...
            return RTResult().failure(RTError(self.pos_start,self.pos_end,"Cannot load script (File not found probably) ",exec_ctx))
    ...
    def execute_tostring(self,exec_ctx):
        if isinstance(exec_ctx.symbol_table.get("value"),Number):
            ...
            return RTResult().failure(RTError(self.pos_start,self.pos_end,f"Parameter is not a string",exec_ctx))
    execute_tostring.arg_names = ['value']
    def execute_modulo(self,exec_ctx):
        if isinstance(exec_ctx.symbol_table.get("value"),Number) and isinstance(exec_ctx.symbol_table.get("value2"),Number):
        
            num = exec_ctx.symbol_table.get("value").value
            num2 = exec_ctx.symbol_table.get("value2").value
...
class Interpreter:
    ...
    def visit_IfNode(self,node,context):
        ...
        for condition, expr, should_return_null in node.cases:  #condition is just a comparison
